...
from src.models import AwardAvailability, LayoverAnalysis, ScoredDeal, TransferBonus
...
def _build_deal_summary_rows(
    ...
    for deal in deals:
        ...
        rows.append({
            ...
            "alt_paths": alt_paths,
            "layovers": layovers,
            "search_url": _build_seats_aero_search_url(a, travelers),
        })
...
def _fmt_hours(h: float) -> str:
    """Format hours without trailing decimals for whole hours."""
    return f"{h:.0f}h" if h == int(h) else f"{h:.1f}h"


def _format_layover_stop(airport: str, city: str, duration_hours: float) -> str:
    """Format a single layover stop for compact email rendering."""
    if city and city.upper() != airport.upper():
        label = f"{city} ({airport})"
    else:
        label = airport or city or "Layover"

    if duration_hours > 0:
        return f"{label} {_fmt_hours(duration_hours)}"
    return label


def _format_stops_summary(avail: AwardAvailability) -> str:
    """Render stop count plus explicit layover airport details."""
    if avail.num_connections == 0:
        summary = "Nonstop"
    else:
        stop_count = f"{avail.num_connections} stop{'s' if avail.num_connections > 1 else ''}"
        layover_stops = [
            _format_layover_stop(layover.airport, layover.city, layover.duration_hours)
            for layover in avail.layovers
        ]
        summary = stop_count
        if layover_stops:
            summary += f" via {' + '.join(layover_stops)}"
        elif avail.max_layover_hours:
            summary += f" · {_fmt_hours(avail.max_layover_hours)} layover"

    if avail.total_travel_hours:
        summary += f" · {_fmt_hours(avail.total_travel_hours)}"

    return summary


def _build_seats_aero_search_url(
    avail: AwardAvailability,
    travelers: int,
) -> str:
    """Generate a best-effort Seats.aero search URL for this route/date."""
    query = urlencode({
        "min_seats": max(travelers, 1),
        "applicable_cabin": "any",
        "disable_live_filtering": "false",
        "date": avail.departure_date.isoformat(),
        "origins": avail.origin,
        "destinations": avail.destination,
    })
    return f"https://seats.aero/search?{query}"
...
def _build_plain_text(
    ...
    if deals:
        ...
        for deal in deals:
            ...
            parts.append(_format_stops_summary(a))
