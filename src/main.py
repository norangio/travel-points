...
async def run_digest() -> None:
    ...
    # Step 5: Rank deals — prioritize focused/high-priority trips first,
    # then use date and score within each trip bucket
...
def _trip_urgency_order(trips: list[dict]) -> dict[str, int]:
    """
    Assign display rank to trips based on configured priority, then date.

    High-priority trips render before broad opportunistic scans even when
    those opportunistic windows start sooner. Within the same priority
    bucket, the nearest departure date shows first.
    """
    ordered = sorted(trips, key=_trip_order_key)
    return {
        trip.get("name", "Unnamed Trip"): index
...
def _trip_scan_order(trips: list[dict]) -> list[dict]:
    ...
    """
    return sorted(trips, key=_trip_order_key)


def _trip_order_key(trip: dict) -> tuple[int, date, str]:
    """Consistent trip ordering for scanning and email display."""
    priority = TRIP_PRIORITY_RANK.get(
        str(trip.get("priority", "medium")).lower(),
        TRIP_PRIORITY_RANK["medium"],
    )
    return (
        priority,
